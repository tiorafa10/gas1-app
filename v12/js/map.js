...
var MapController = (function () {
  ...
  function init() {
    ...
    // Mostrar/esconder labels dos postes conforme zoom
    map.on('zoomend', function () {
      var show = map.getZoom() >= 16;
      map.getContainer().classList.toggle('show-pole-labels', show);
    });
  ...
  function addPoleMarkers(poles, progressMap) {
    ...
    poles.forEach(function (pole) {
      ...
      // Label permanente (visivel apenas em zoom >= 16)
      marker.bindTooltip(pole.parque + ' · ' + (pole.barramento || pole.id), {
        permanent: true,
        direction: 'top',
        className: 'pole-tooltip pole-label'
      });
